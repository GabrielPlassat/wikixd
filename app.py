...
def charger_ressources():
    ...
    return data["df"], data["embeddings"], anthropic_client, openai_client

df, embeddings, anthropic_client, openai_client = charger_ressources()
...
def recherche_hybride(question, top_n=8):
    ...
    scores_final = 0.5 * scores_sem + 0.5 * scores_kw
    top_idx = np.argsort(scores_final)[::-1][:top_n]
    res = df.iloc[top_idx].copy()
    res['score'] = scores_final[top_idx]
    return res
